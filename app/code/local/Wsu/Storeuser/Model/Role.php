...
/*
 * Use as singleton
 * Mage::getSingleton('storeuser/role')
 */
/**
 * @refactor
 * make 3 subclasses - Strategy pattern
 * Wsu_Storeuser_Model_Role_StoreLimited
 * Wsu_Storeuser_Model_Role_WebsiteLimited
 * Wsu_Storeuser_Model_Role_NotLimited - Null Object pattern
 * exclude all checks [if ($this->isScopeStore())] and [if ($this->isScopeWebsite())]
 */
class Wsu_Storeuser_Model_Role {
    ...
    public function __construct() {
        $this->_helper = Mage::helper('storeuser');
    }
    ...
    private function _getRecordCollection() {
        if (null == $this->_recordCollection) {
            $this->_recordCollection = Mage::getModel('storeuser/advancedrole')->getCollection()->loadByRoleId($this->_getCurrentRoleId());
        }
