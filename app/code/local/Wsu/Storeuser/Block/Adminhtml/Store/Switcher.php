<?php
class Wsu_Storeuser_Block_Adminhtml_Store_Switcher extends Mage_Adminhtml_Block_Catalog_Category_Tree // Mage_Adminhtml_Block_Store_Switcher
    {
    /**
     * @var array
     */
    ...
    public function __construct() {
        parent::__construct();
        $this->setTemplate('storeuser/store_switcher.phtml');
    ...
    public function getStoreIds() {
        if (!isset($this->_storeIds)) {
            ...
            $currentRoles    = Mage::getModel('storeuser/advancedrole')->getCollection()->loadByRoleId(Mage::app()->getRequest()->getParam('rid'));
    ...
    protected function _getNodeJson($node, $level = 0, $storeId = null) {
        ...
        $aCategories = array();
        if ($storeId) {
            $currentRoles = Mage::getModel('storeuser/advancedrole')->getCollection()->loadByRoleId($this->getRequest()->getParam('rid'));
            foreach ($currentRoles as $role) {
    ...
    public function getLoadTreeUrl($expanded = null) {
        return $this->getUrl('storeuser/adminhtml_categories/categoriesJson', array(
            '_current' => true
        ));
    }
