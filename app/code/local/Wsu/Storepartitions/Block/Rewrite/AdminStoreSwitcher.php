<?php
class Wsu_Storepartitions_Block_Rewrite_AdminStoreSwitcher extends Mage_Adminhtml_Block_Store_Switcher {
    ...
    protected function _toHtmlReports() {
        ...
        if ($allowedWebsiteIds && $allowedStoreIds && $AllowedStoreviewIds) {
            if (in_array($currentWebsiteId, $allowedWebsiteIds) || in_array($currentStoreId, $allowedStoreIds) || array_intersect($currentStoreviewIds, $AllowedStoreviewIds)) {
                $bAllow = true;
            }
        }
        if (!$bAllow) {
            $url = Mage::getModel('adminhtml/url')->getUrl('*/*/*', array(
                '_current' => false,
                'store' => $AllowedStoreviewIds[0]
            ));
        ...
        if ($role->isPermissionsEnabled()) {
            ...
            if (!empty($AllowedStoreviews)) {
                if (!in_array(Mage::app()->getRequest()->getParam('store'), $AllowedStoreviews)) {
                    ...
                    Mage::app()->getResponse()->setRedirect($url->getUrl('*/*/*', array(
                        '_current' => true,
                        'store' => $AllowedStoreviews[0]
                    )));
                }
            }
            // removing <option value="">All Store Views</option> option if have limited access
