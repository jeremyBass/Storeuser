<?php
class Wsu_Storepartitions_Block_Adminhtml_Permissions_Tab_Product_Editor_Attribute extends Mage_Eav_Block_Adminhtml_Attribute_Grid_Abstract {
    protected $_roleId = 0;
    ...
    public function getGridUrl() {
        ...
        return $this->getUrl('storepartitions/adminhtml_editor/attributegrid', array('rid' => $roleId));
    }
    ...
    protected function _prepareCollection() {
        ...
        $collection->getSelect()
            ->columns(
                array('is_sp_allow'=>new Zend_Db_Expr ('IF( is_global IN ( '.implode(',', $this->_getEnableScope()).' ) , 1, 0 )'))
            )
            ->joinLeft(
    ...
    protected function _prepareColumns() {
        ...
        $this->addColumn('is_allow', array(
            'type'      => 'radio',
            'header'    =>Mage::helper('storepartitions')->__('Allow'),
            'name'      => 'is_allow',
            ...
            'renderer'  => 'Wsu_Storepartitions_Block_Adminhtml_Permissions_Tab_Product_Editor_Render_Radio'
        ));

        $this->addColumn('is_disable', array(
            'type'      => 'radio',
            'header'    =>Mage::helper('storepartitions')->__('Deny'),
            'name'      => 'is_disable',
        ...
        $this->addColumn('is_sp_allow', array(
            'header'=>Mage::helper('storepartitions')->__('Inherited from Scope'),
            'sortable'=>true,
            'index'=>'is_sp_allow',
            'name' => 'is_sp_allow',
            'field_name'=> 'is_sp_allow',
            'type' => 'options',
            'width' => '100px',
        ...
        $this->addColumn('is_global', array(
            'header'=>Mage::helper('storepartitions')->__('Scope'),
            'sortable'=>true,
            'index'=>'is_global',
            'type' => 'options',
        ...
        $this->addColumn('attribute_group_name', array(
            'header'=>Mage::helper('storepartitions')->__('Tabs'),
            'sortable'=>true,
    ...
    protected function _getAllowAttribute($json=false){
        ...
        $attrEnable  = Mage::getModel('storepartitions/editor_attribute')->getRoleAttributeEnable($this->_getRoleId());
    ...
    protected function _getDisableAttribute($json=false){
        ...
        $attrDisable  = Mage::getModel('storepartitions/editor_attribute')->getRoleAttributeDisable($this->_getRoleId());
    ...
    protected function _getDisableScopeOptions(){
        return array(
                1 =>Mage::helper('storepartitions')->__('Allow'),
                0 =>Mage::helper('storepartitions')->__('Deny'),
            );
    }
    ...
    protected function _getEnableScope() {
        if(!($scope = $this->getRequest()->getParam('scope'))) {
            ...
            $canEditGlobal = Mage::getModel('storepartitions/advancedrole')->canEditGlobalAttributes($this->_getRoleId());
        }else{
